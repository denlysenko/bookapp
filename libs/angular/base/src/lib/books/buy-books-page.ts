...
export abstract class BuyBooksPageBase extends BaseComponent {
  readonly #storeService = inject(StoreService);
  readonly #booksService = inject(BooksService);

  readonly filter = signal<BooksFilter>(
    this.#storeService.get('BUY_BOOKS') || {
      searchQuery: '',
  ...
  readonly source$ = this.#booksService
    .watchBooks(true, this.filterInput, this.filter().sortValue)
  ...
  #updateFilterInStore() {
    this.#storeService.set('BUY_BOOKS', this.filter());
  }
}
